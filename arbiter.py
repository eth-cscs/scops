...
class ScopsArbiter:
    ...
    def cleanupRequestsOffers(self, requests, offers, nodetype):
    ...
    def createTransferRequests(self, requests, offers, nodetype, transfer_requests, algo, iteration):
        idx_o = 0
        idx_r = 0
        ...
        while idx_r < len(s_requests) and idx_o < len(s_offers):
            ...
            if nodes_o <= nodes_r:
                ...
                s_offers[idx_o][1] = 0
                req={ 'nnodes': nodes_o, 'init_nnodes': nodes_o, 'nodelist':'', 'node_type': nodetype, 'id': str(uuid.uuid4()), 'src': name_o, 'dst': name_r, 'status': 'created', 'algo': algo, 'iteration': iteration }
                ...
                req={ 'nnodes': nodes_r, 'init_nnodes': nodes_r, 'nodelist':'', 'node_type': nodetype, 'id': str(uuid.uuid4()), 'src': name_o, 'dst': name_r, 'status': 'created', 'algo': algo, 'iteration':iteration }
                transfer_requests.append(req)
                s_requests[idx_r][1] = 0
                idx_r+=1
        return s_requests, s_offers, transfer_requests

    def factorizeTranferRequests(self, transfer_requests, node_types):
    ...
    def distributeResources(self, usage, clusters, global_node_types, iteration):
        ...
        print("--- offers of not loaded clusters and predicates ---")
        ...
        for n in global_node_types:
            print("offers of not loaded clusters ",n, offers[n])
        ...
        for n in global_node_types:
            ...
            for name in pending_loaded_clusters[n]:
                ...
                # test predicate
                if clusters[name].evalPredicate(active_c_n, n):
                    # offers
        ...
        # clean-up requests
        for n in global_node_types:
            requests[n], offers[n] = self.cleanupRequestsOffers(requests[n], offers[n], n)

        # check offers/requests
        for n in global_node_types:
            print("predicate values ",n, pred_values[n])
            print("predicate offers ",n, offers[n])
            print("predicate requests ",n, requests[n])
        ...
        for n in global_node_types:
            ...
            for name,req_size in requests[n]:
                if all_requests_size > 0:
                   nrr = req_size / all_requests_size * clusters[name].priority_w[0]
                else:
                    nrr = 0
                fairshare_exp= - (usage[name][n]['running']/self.total_resource[n])/clusters[name].share[n]
                fairshare = 2**fairshare_exp * clusters[name].priority_w[1]
                values.append(int(Parameters.alpha+nrr+fairshare))
                weights.append(req_size)
            # knapsack
            picks, max_val = knapsack_dp(values, weights, len(requests[n]), total_offer[n])
        ...
        print("--- create transfer requests ---")
        #for n in global_node_types:
        ...
        for n in global_node_types:
            if n in selected_requests.keys() and n in offers.keys():
               selected_requests[n], offers[n], transfer_requests = self.createTransferRequests(selected_requests[n], offers[n], n, transfer_requests, 'knapsack', iteration)
        print("TF knapsack:", len(transfer_requests))
        for k in transfer_requests:
            print(k['src'], k['dst'], k['node_type'], k['nnodes'])

        print("--- proportional distribution ---")
        ...
        # check offers/requests
        for n in global_node_types:
            print("proportional offers ",n, new_offers[n])
            print("proportional requests ",n, new_requests[n])
        # clean-up requests
        for n in global_node_types:
            new_requests[n],  new_offers[n] = self.cleanupRequestsOffers(new_requests[n], new_offers[n], n)

        for n in global_node_types:
            if n in new_requests.keys() and n in new_offers.keys():
                new_requests[n], new_offers[n], transfer_requests = self.createTransferRequests(new_requests[n], new_offers[n], n, transfer_requests, 'proportional', iteration)
        print("TF proportional:", len(transfer_requests))
        for k in transfer_requests:
            print(k['src'], k['dst'], k['node_type'], k['nnodes'])

        #print("--- factorize transfer_requests ---")
        # factorise transfer requests
        #f_transfer_requests = self.factorizeTranferRequests(transfer_requests, global_node_types)
