...
def ConstValue(usage, n, now, values):
    return values[0]

def ConstValueIfPending(usage, n, now, values):
...
def AvgJobWaitTime(usage, n, now, values):
    if 'avg_job_wait_time' not in usage:
        return True
    return usage['avg_job_wait_time'] < values[0]

def QuotaAvailableAllowBigJob(usage, n, now, values):
    if 'max_pending' not in usage or 'pending' not in usage or 'cap' not in usage:
        return True
    quota = values[0]
    # allow big job
